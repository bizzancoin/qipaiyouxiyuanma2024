<template>
  <div class="app-container calendar-list-container" v-loading="loading">
    <div class="filter-container">
      房间：<el-select size="small" style="width:180px" @change="handleRoomChange" placeholder="请选择" v-model="roomId" clearable>
    ...
    <el-table :key="strategyKey" size="mini" :data="curPrizePool.strategys" border fit style="width:100%;margin-top:30px;">
      ...
      <el-table-column align="center" prop="control.winRatio" label="玩家获胜概率" width="100">
        <template slot-scope="scope">
          {{scope.row.control.winRatio}}%
        </template>
      </el-table-column>
    ...
    <el-dialog :title="addStrategy.title" :visible.sync="addStrategy.visible">
      <el-form :model="addStrategy.strategy" :rules="addStrategyRules" ref="addStrategyForm" label-width="280px" class="number-hide">
        ...
        <el-form-item label="玩家获胜概率" prop="winRatio">
          <el-input size="small" style="width:160px" v-model.number="addStrategy.strategy.winRatio" 
            type="number" min="0" max="100" clearable/>
          <span style="margin-left:5px">%</span>
        </el-form-item>
        <el-form-item style="margin-top:40px">
    ...
    return {
      ...
      addStrategy: {
        ...
        init_1: function(strategy) {
          ...
          if (strategy) {
            this.strategy.winRatio = strategy.control.winRatio;
          }
        }
      },
      addStrategyRules: {
  ...
  methods: {
    ...
    handleAddPrizePool(isDef) {
      ...
      if (isDef) {
        ...
        this.$prompt("请输入奖池金额", "添加默认奖池", {
          ...
          .then(({ value }) => {
            this.handleAddDefPrizePool(value * 1000);
          })
          .catch(() => {});
      } else {
      ...
      this.$refs.addPoolForm.validate(valid => {
        if (valid) {
          ...
          let pool = {
              gameId: this.gameId,
              roomId: this.roomId,
              prizePools: this.addPool.pool.prizePools * 1000,
              lower: this.addPool.pool.lower * 1000,
              upper: this.addPool.pool.upper * 1000
          }
    ...
    handleModPrizePoolGold(isDef, gold, id) {
      this.$prompt("请输入奖池金额", "修改奖池", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        inputValue: gold/1000,
        inputPattern: /^-?[0-9]*.?[0-9]*$/,
        inputErrorMessage: "请输入整数"
      })
        .then(({ value }) => {
          if (isDef == 1) {
            this._modDefPrizePoolGold(value * 1000);
          } else {
            if (id) {
              this._modGoldPrizePoolGold(value * 1000, id);
            }
          }
        })
        .catch(() => {});
    ...
    handleAppendPrizePoolGold(isDef, id) {
      this.$prompt("请输入奖池金额", "追加奖池", {
        ...
        .then(({ value }) => {
          if (isDef == 1) {
            this._appendDefPrizePoolGold(value * 1000);
          } else {
            if (id) {
              this._appendGoldPrizePoolGold(value * 1000, id);
            }
          }
        })
        .catch(() => {});
      ...
      this.$refs.changeRangeForm.validate(valid => {
        if (valid) {
          ...
          let pool = {
              gameId: this.gameId,
              roomId: this.roomId,
              prizePoolId: this.changeRange.pool.id,
              lower: this.changeRange.pool.lower * 1000,
              upper: this.changeRange.pool.upper * 1000
          }
    ...
    _handleDefPrizePoolStrategy(isEdit, func) {
      this.addStrategy.strategy.control.loseRatio =
        100 - this.addStrategy.strategy.winRatio;
      this.addStrategy.strategy.control.winRatio = this.addStrategy.strategy.winRatio;
      if (isEdit) {
        modDefPrizePoolStrategy(
          this.gameId,
          this.roomId,
