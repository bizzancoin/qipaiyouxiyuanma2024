<template>
  <div class="app-container calendar-list-container" v-loading="loading">
    ...
    <el-table :key="strategyKey" size="mini" :data="curPrizePool.strategys" border fit style="width:100%;margin-top:30px;">
      ...
      <el-table-column align="center" prop="range" label="奖池范围" width="300" >
        <template slot-scope="scope">
          {{scope.row.lower | goldToMoney}} ~ {{scope.row.upper | goldToMoney}}
        </template>
      </el-table-column>
      <el-table-column align="center" label="范围" width="400">
        <template slot-scope="scope">
          <table width="100%">
              <tr v-for="(item,index) in scope.row.control.difficultys" :key="index">
                <td>鱼的倍数: {{item.multipleLower}}~{{item.multipleUpper}}</td>
                &nbsp;&nbsp;&nbsp;
                <td>难易程度：{{difficultyName(item)}}</td>
                <td>
                  <el-button v-if="hasPermission('D0102')" size="small" type="danger" 
                    @click="handleDeleteStrategyRange(scope.row, index)">删除</el-button>
                </td>
              </tr>
          </table>
        </template>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope" v-if="hasPermission('D0102')">
          ...
          <el-button size="small" type="primary" @click="handleOpenAddStrategyRange(scope.row)" >添加范围</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 添加策略范围 -->
    <el-dialog :title="strategyRange.title" :visible.sync="strategyRange.visible">
      <el-form :model="strategyRange.range" :rules="strategyRangeRules" ref="strategyRangeForm" label-width="280px" class="number-hide">
        <el-form-item label="鱼倍数范围">
          <el-col :span="7">
            <el-form-item prop="multipleLower">
              <el-input size="small" style="width:160px" v-model.number="strategyRange.range.multipleLower"
              type="number" clearable/>
            </el-form-item>
          </el-col>
          <el-col :span="1">~</el-col>
          <el-col :span="8">
            <el-form-item prop="multipleUpper">
              <el-input size="small" style="width:160px"  v-model.number="strategyRange.range.multipleUpper" 
              type="number"  clearable/>
            </el-form-item>
          </el-col>
        </el-form-item>
        <el-form-item label="难易程度">
          <el-select size="small" style="width:180px" placeholder="请选择" v-model="strategyRange.range.difficulty">
        ...
        <el-form-item style="margin-top:40px">
          <el-button size="small" @click.native="strategyRange.visible = false">取消</el-button>
          <el-button size="small" type="primary" @click.native="handleAddStrategyRange">提交</el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
...
export default {
  ...
  data() {
    ...
    var validateStrategyMultipleUpper = (rule, value, callback) => {
      if (value === "" || !Number.isInteger(value)) {
        callback(new Error("请输入最高倍数"));
      } else {
        if(this.strategyRange.range.multipleUpper <= 0) {
          callback(new Error("最高倍数必须大于0"));
        }
        if (this.strategyRange.range.multipleUpper < this.strategyRange.range.multipleLower) {
          callback(new Error("最高倍数必须大于最低倍数"));
        }
        callback();
      }
    };
    ...
    return {
      ...
      strategyRangeRules: {
        ...
        multipleUpper: [
          {
            min: 1,
            validator: validateStrategyMultipleUpper,
            type: "number",
            trigger: "blur"
          }
        ]
  ...
  methods: {
    ...
    findRoomList() {
      ...
      getRoomList({ gameId: this.gameId, active: 1 })
        .then(res => {
          ...
          if(this.roomOptions && this.roomOptions.length){
    ...
    difficultyName(row) {
      if (this.difficultOptions) {
        for (let i = 0; i < this.difficultOptions.length; i++) {
          if (this.difficultOptions[i].id == row.difficulty) {
    ...
    convertPrizePoolStrategys(prizePool) {
      ...
      for (let i = 0; i < prizePool.strategys.length; i++) {
        ...
        if (prizePool.strategys[i].control.difficultys == null) {
          prizePool.strategys[i].control.difficultys = [];
        }
      }
    ...
    // 打开添加范围
    handleOpenAddStrategyRange(strategy) {
      if (strategy) {
        this.strategyRange.def = this.curPrizePool.def;
        this.strategyRange.prizePoolId = this.curPrizePool.id;
        this.strategyRange.strategy = {
          id: strategy.id,
          control: strategy.control,
          gold: goldToMoneyNotFixed(strategy.gold),
          lower: goldToMoneyNotFixed(strategy.lower),
        ...
        this.strategyRange.visible = true;
        this.strategyRange.range.difficulty = this.difficultOptions[0].id;
      }
    },
    _handleAddStrategyRange(isDef, strategy, func) {
      if (isDef) {
        modDefPrizePoolStrategy(this.gameId, this.roomId, strategy)
          .then(res => {
            this.loading = false;
            func();
            this.$message({
    ...
    // 添加策略范围
    handleAddStrategyRange() {
      this.$refs.strategyRangeForm.validate(valid => {
        if (valid) {
          // 默认奖池策略
          if (
            this.strategyRange.strategy.control.difficultys == null ||
            this.strategyRange.strategy.control.difficultys.length <= 0
          ) {
            this.strategyRange.strategy.control.difficultys = [];
          }
          let range = {
            multipleLower: this.strategyRange.range.multipleLower,
            multipleUpper: this.strategyRange.range.multipleUpper,
            difficulty: this.strategyRange.range.difficulty
          };
          this.strategyRange.strategy.control.difficultys.push(range);
          if (this.strategyRange.def == 1) {
            this._handleAddStrategyRange(
              true,
              this.strategyRange.strategy,
              () => {
                this.strategyRange.visible = false;
                this.$refs["strategyRangeForm"].resetFields();
              }
            );
          } else {
            // 金币奖池策略
    ...
    handleDeleteStrategyRange(strategy, idx) {
      let diffs = [];
      for (let i = 0; i < strategy.control.difficultys.length; i++) {
        if (i != idx) {
          diffs.push(strategy.control.difficultys[i]);
        }
      }
      let tmp = {
        id: strategy.id,
      ...
      this.$confirm("是否删除此范围?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        if (this.curPrizePool.def == 1) {
          this._handleAddStrategyRange(true, tmp, () => {});
        } else {
          // 金币奖池策略
          this._handleAddStrategyRange(false, tmp, () => {});
        }
      });
    },
