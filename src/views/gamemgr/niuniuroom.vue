<template>
  <div class="app-container calendar-list-container">
    ...
    @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="baseBet" label="底注" width="120">
        </el-table-column>
        <el-table-column label="下注倍数" width="250">
            <template slot-scope="scope">
                <el-tag v-for="tag in scope.row.betOptions" :key="tag">
                {{tag}}
                </el-tag>
            </template>
        </el-table-column>
    ...
    <el-dialog :title="addFormTitle" :visible.sync="addFormVisible" >
        <el-form :model="addForm" label-width="220px" :rules="addFormRules" ref="addForm" class="number-hide">
            ...
            <el-form-item label="底注" prop="baseBet">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.baseBet" auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="下注倍数" prop="betOptions">
                <el-tag :key="tag" v-for="tag in addForm.betOptions" closable :disable-transitions="false" @close="handleDelTag(tag)">
                    {{tag}}
                </el-tag>
...
import { getRoomType,getRoomList,saveGameRoom,changeRoomStatus,delGameRoom } from "@/api/games/niuniuroom";
  ...
  data() {
    return {
        ...
        addFormRules: {
            ...
            baseBet: [{ type:'number',required: true, min: 1, message: "请输入底注", trigger: "change" }],
