<template>
  <div class="app-container calendar-list-container">
    ...
    @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="maxBet" label="最高下注" width="120">
        </el-table-column>
        <el-table-column label="下注选项" width="180">
            <template slot-scope="scope">
                <el-tag v-for="tag in scope.row.betList" :key="tag">
                {{tag}}
                </el-tag>
            </template>
        </el-table-column>
    ...
    <el-dialog :title="addFormTitle" :visible.sync="addFormVisible" >
        <el-form :model="addForm" label-width="220px" :rules="addFormRules" ref="addForm" class="number-hide">
            ...
            <el-form-item label="最高下注" prop="maxBet">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.maxBet" auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="下注选项" prop="betList">
                <el-tag :key="tag" v-for="tag in addForm.betList" closable :disable-transitions="false" @close="handleDelTag(tag)">
                    {{tag}}
                </el-tag>
                <el-input class="input-new-tag" v-if="addBetInputVisible" v-model.number="addInputBetValue" ref="saveTagInput" size="small"
                @keyup.enter.native="handleInputConfirm"  @blur="handleInputConfirm"></el-input>
                <el-button v-else class="button-new-tag" size="small" @click="showInput">+ 新项</el-button>
            </el-form-item>
...
import { getRoomType,getRoomList,saveGameRoom,changeRoomStatus,delGameRoom } from "@/api/games/blackjackroom";
...
export default {
  ...
  data() {
    return {
        ...
        addFormRules: {
            ...
            maxBet: [{ type:'number',required: true, message: "请输入最高下注筹码", trigger: "change" }],
            betList: [
                { required: true, message: '请输入下注倍数', trigger: 'change' }
            ],
  ...
  methods: {
    ...
    handleDelTag(tag) {
        this.addForm.betList.splice(this.addForm.betList.indexOf(tag), 1);
    },
    showInput() {
        this.addBetInputVisible = true;
        this.$nextTick(_ => {
            this.$refs.saveTagInput.$refs.input.focus();
        });
    },
    handleInputConfirm() {
        if(this.addForm.betList.length == 5){
            this.$message({
                message: "最多添加5项",
                type: "warn"
            });
            ...
            let addInputBetValue = this.addInputBetValue;
            if (addInputBetValue) {
                this.addForm.betList.push(parseInt(addInputBetValue));
                this.addForm.betList.sort((a,b)=>{
                    return a>b;
                });
                console.log(this.addForm.betList)
            }
        }
        this.addBetInputVisible = false;
        this.addInputBetValue = '';
    },
