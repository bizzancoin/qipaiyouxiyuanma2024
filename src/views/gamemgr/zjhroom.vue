<template>
  <div class="app-container calendar-list-container">
    ...
    @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="base" label="底注" width="120">
        </el-table-column>
        <el-table-column prop="top" label="封顶" width="120">
        </el-table-column>

        <el-table-column label="下注选项" width="200">
            <template slot-scope="scope">
                <el-row>选项1：<el-tag >{{scope.row.chip1}}</el-tag></el-row>
                <el-row>选项2：<el-tag >{{scope.row.chip2}}</el-tag></el-row>
                <el-row>选项3：<el-tag >{{scope.row.chip3}}</el-tag></el-row>
            </template>
        </el-table-column>
    ...
    <el-dialog :title="editFormTitle" :visible.sync="addFormVisible" >
        <el-form :model="addForm" label-width="220px" :rules="addFormRules" ref="addForm" class="number-hide">
            ...
            <el-form-item label="默认最低筹码" prop="minOne">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.minOne" auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="默认最高筹码" prop="maxOne">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.maxOne" auto-complete="off"></el-input>
            </el-form-item>

            <el-form-item label="底注" prop="base">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.base" auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="封顶" prop="top">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.top" auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="下注选项1" prop="chip1">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.chip1" auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="下注选项2" prop="chip2">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.chip2" auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="下注选项3" prop="chip3">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.chip3" auto-complete="off"></el-input>
            </el-form-item>
...
import { getRoomType,getRoomList,saveGameRoom,changeRoomStatus,delGameRoom } from "@/api/games/zjhroom";
...
export default {
  ...
  data() {
    return {
        ...
        addLoading: false,
        addFormRules: {
            name: [{ required: true, message: "请输入房间名", trigger: "change" },{ min: 3, max: 20, message: '长度在 3 到 20 个字符', trigger: 'change' }],
            maxNum: [{ type:'number',required: true, message: "请输入最大人数", trigger: "change" },{ type:'number',min: 1, message: '至少1', trigger: 'change' }],
            tableNum: [{ type:'number',required: true, message: "请输入桌子数", trigger: "change" },{ type:'number',min: 1, message: '至少1', trigger: 'change' }],
            chair: [{ type:'number',required: true, message: "请输入每桌人数", trigger: "change" },{ type:'number',min: 1, message: '至少1', trigger: 'change' }],
            lower: [{ type:'number',required: true, message: "请输入进入下限", trigger: "change" }],
            upper: [{ type:'number',required: true, message: "请输入进入上限", trigger: "change" }],
            free: [{ type:'number',required: true, message: "请输入空闲人数百分比", trigger: "change" },{ type:'number',min: 1, max: 100, message: '请输入1-100之间的整数', trigger: 'change' }],
            general: [{ type:'number',required: true, message: "请输入普通人数百分比", trigger: "change" },{ type:'number',min: 1, max: 100, message: '请输入1-100之间的整数', trigger: 'change' }],
            crowded: [{ type:'number',required: true, message: "请输入拥挤人数百分比", trigger: "change" },{ type:'number',min: 1, max: 100, message: '请输入1-100之间的整数', trigger: 'change' }],
            ordinarPeople: [{ type:'number',required: true, message: "请输入普通会员进入人数百分比", trigger: "change" },{ type:'number',min: 1, max: 100, message: '请输入1-100之间的整数', trigger: 'change' }],
            afee: [{ type:'number',required: true, message: "请输入台费", trigger: "change" }],
            minOne: [{ type:'number',required: true, message: "请输入默认最低筹码", trigger: "change" }],
            maxOne: [{ type:'number',required: true, message: "请输入默认最高筹码", trigger: "change" }],
            base: [{ type:'number',required: true, message: "请输入底注", trigger: "change" }],
            top: [{ type:'number',required: true, message: "请输入封顶", trigger: "change" }],
            chip1: [{ type:'number',required: true, message: "请输入下注选项1", trigger: "change" }],
            chip2: [{ type:'number',required: true, message: "请输入下注选项2", trigger: "change" }],
            chip3: [{ type:'number',required: true, message: "请输入下注选项3", trigger: "change" }],
            proportionGold: [{ type: "number", required: true, message: "请输入金币兑比例", min: 1, trigger: "change"}],
            proportionChips: [{ type: "number", required: true, message: "请输入筹码兑换比例", min: 1, trigger: "change"}]
        },
        editFormTitle:"新建房间",
  ...
  methods: {
    ...
    handleDelTag(tag) {
        this.addForm.betOptions.splice(this.addForm.betOptions.indexOf(tag), 1);
    },
    showInput() {
    ...
    handleInputConfirm() {
        let inputBetValue = this.inputBetValue;
        if (inputBetValue) {
            this.addForm.betOptions.push(inputBetValue);
        }
