<template>
  <div class="app-container calendar-list-container">
    ...
    @selection-change="handleSelectionChange">
        ...
        <el-table-column label="机器人分数" width="200">
            <template slot-scope="scope">
                <span>{{scope.row.robotLower}}~{{scope.row.robotUpper}}</span>
            </template>
        </el-table-column>
    ...
    <el-dialog :title="addFormTitle" :visible.sync="addFormVisible" >
        <el-form :model="addForm" label-width="220px" :rules="addFormRules" ref="addForm" class="number-hide">
            ...
            <el-form-item label="机器人下限" prop="robotLower">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.robotLower" auto-complete="off"></el-input>
            </el-form-item>
            <el-form-item label="机器人上限" prop="robotUpper">
                <el-input type="number" placeholder="输入整数" v-model.number="addForm.robotUpper" auto-complete="off"></el-input>
            </el-form-item>
            ...
            <el-form-item label="单线押注" prop="betList">
                <el-tag :key="tag" v-for="tag in addForm.betList" closable :disable-transitions="false" @close="handleDelTag(tag)">
                    {{tag}}
                </el-tag>
                <el-input class="input-new-tag" v-if="betInputVisible" v-model.number="inputBetValue" ref="saveTagInput" size="small"
...
import { getRoomType,getRoomList,saveGameRoom,changeRoomStatus,delGameRoom } from "@/api/games/fruitsroom";
...
export default {
  ...
  data() {
    return {
        ...
        addFormRules: {
            ...
            robotLower: [{ type:'number',required: true, message: "请输入机器人分数下限", trigger: "change" }],
            robotUpper: [{ type:'number',required: true, message: "请输入机器人分数上限", trigger: "change" }],
            ...
            betList: [
                { required: true, message: '请输入单线押注', trigger: 'change' }
            ],
  ...
  methods: {
    ...
    handleInputConfirm() {
        ...
        if (inputBetValue) {
            this.addForm.betList.push(inputBetValue);
        }
